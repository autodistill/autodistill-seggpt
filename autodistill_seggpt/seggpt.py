...
class SegGPT(DetectionBaseModel):
    ...
    def prepare_ref_img(self,img:np.ndarray,detections:Detections):
        ...
        for detection in detections:
            ...
            if not (mh == ih and mw == iw):
                ...
                raise NotImplementedError("mask resizing is disabled--seems to cause kernel crashes.")
                det_mask = cv2.resize(det_mask, dsize=(iw,ih))
                mh,mw = det_mask.shape
            assert mh == ih and mw == iw, f"mask shape {det_mask.shape} does not match image shape {og_img.shape}. new image shape is {img.shape}"
    ...
    def prepare_ref_imgs(self, cls_name: str, refs:List[Tuple[np.ndarray,Detections]]):
    ...
    @torch.no_grad()
    def predict(self,input:Union[str,np.ndarray], _confidence:int = 0.5) -> sv.Detections:
        ...
        for keyId,raw_ref_imgs in enumerate(self.ontology.rich_prompts()):
            ...
            ref_imgs,ref_masks,min_ref_area = self.prepare_ref_imgs(keyId,raw_ref_imgs)
            ...
            for i in range(len(imgs)):
                cv2.imwrite(f"debug/img_{i}.png",(imgs[i]*imagenet_std+imagenet_mean)*255)
                cv2.imwrite(f"debug/mask_{i}.png",(masks[i]*imagenet_std+imagenet_mean)*255)
